...
def get_news(request):
    ...
    try:
        ...
        news_data = news_service.get_financial_news(symbol=symbol if symbol else None)
        
        return Response(news_data)
        
    except Exception as e:
        logger.error(f"Error in news view: {e}")
        return Response({"error": "Unable to retrieve financial news"}, status=500)
